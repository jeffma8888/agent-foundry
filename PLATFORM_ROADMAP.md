...
STATUS (iter 133): current through iteration 133; its PM wrote the ledger row and a later stage appended the archive bullet in the ship commit. It took item (f),
now SHIPPED; (b), (c), (d), (e), (g)'s `parse_triage_winner` half and (h) remain open.
...
whoever crosses it turns the suite RED and gets REVERTED for a docs-only reason. **RE-MEASURED iter 133, post-edit: this index is 56020 chars, leaving 3980. Iteration 133 is net -135: it
retired item (f)'s paragraph (-215) and collapsed three redundant STATUS lines, which more than paid
for its own two records. (The iter-128
