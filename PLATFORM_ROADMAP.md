...
the same iteration as 127 itself (a live loop holds the pre-127 `foundry.py` in memory). (e) NEW, created by iteration 129: decide whether the `"stalled"` failure kind (`Connection stalled -- no data received`, 9 of the live window's 57 zero-output attempts, 9,600 s of sleep) belongs on the FAST retry ladder. Iteration 129 left it on the long `BACKOFFS` ladder on purpose because the operator evidence that stalls are a per-turn stall rather than a network fault is suggestive but unmeasured; naming it as its own kind is what makes the question decidable. **(e) is now MEASURED by iteration 130's scout A and is the top ready-to-ship item:** the 9 stalled attempts die at a MEDIAN of 359 s (range 306-405), not at the 120 s the watchdog message names, so a stall destroys ~6 min of productive work rather than merely waiting; 5 of the 6 non-exhausted stalled stage-runs recovered on the VERY NEXT attempt; and the one exhausted case stalled on all 4 attempts DESPITE paying 600+1200+2400 s, so the long ladder neither rescued the persistent case nor was needed by the transient one. Shape = a per-kind ladder MAP consulted by `retry_delay` BEFORE its existing fast/long choice (keeping retry 1 cheap and retries 3-4 slow), NOT moving `stalled` wholesale into `FAST_RETRY_KINDS`. (f) DEFINE THE 4 UNDEFINED SCOUT LENSES in `roles/pm_scout.md`: `PM_SCOUT_LENS_POOL` holds 6 and is WIRED, but the card defines exactly 2 and still says "do not propose candidates from the other lens", so only 1 iteration in 6 runs a fully-documented pair and 3 in 6 run BOTH scouts on an undefined lens -- iteration 130 was one of those. Also de-hardcode the retired fixed a/b mapping asserted as fact in `roles/pm.md`, `ARCHITECTURE.md` and `docs/DUAL_PM_SCOUT_SPEC.md`, and add a suite test asserting every pool entry is defined in the card so a 7th lens cannot be added undocumented. (g) widen the decision-log parsers: `parse_scout_candidates` accepts only `## Candidate `, so 8 of 54 scout slates render as ZERO candidates with no sign of the loss, and `parse_triage_winner` leaves 4 of 27 rows reading `winner: unknown`; render "present but unparsed" distinctly from "absent". (h) paper-cut: `roles/pm.md` mandates `foundry lint-spec`, which is NOT on PATH inside a product stage -- VERIFIED this run that `python3 foundry.py lint-spec --file <path>` works from the foundry checkout root, so the card should carry that exact invocation for a non-foundry cwd. **DE-LISTED by iteration 130's scout A, measured -- do not re-propose:** cutting `MAX_ATTEMPTS` for the timeout kind (12 of 17 timeout stage-runs RECOVER on a retry, so it trades 5.83 h of visible waste for lost iterations); and an "output file is stable, so stop the attempt" early exit (the 7.01 h of post-last-write agent time is concentrated in engineer and final, whose real work lands in git rather than in the state file, so stopping there destroys code and commits). ALSO STILL OPEN: the iteration-121 RETRY (its 51,824-byte
