...
STATUS (iters 130-134): current through iteration 134; each PM wrote its own two records in the ship
commit (iteration 133's archive bullet was appended by a later stage of the same commit). Items (a),
(f) and (b) shipped in iterations 132, 133 and 134 respectively, each off a scout slate; STILL OPEN:
(c), (d), (e), (g)'s `parse_triage_winner` half, (h), and the NEW (i) below.
