...
# 40. Repetition brake: sample the recent commit subjects + newest roadmap entries, normalize each into a "shape" (conventional-commit prefix + trailing (... iter N) tag stripped), and flag a RUT when any single shape recurs at least NOVELTY_RUT_THRESHOLD times -- the "twelve near-identical increments" drift the discovery loop is meant to catch -- else VARIED; The CLI verb is read-only and writes nothing (exit 0 VARIED / 1 RUT). Its CORE is LIVE (call site measured by `ast` in iter 208, NOT dormant): `build_prompt` interpolates `pm_novelty_block`, which calls `gather_novelty` + `novelty_advice`, so EVERY stage prompt already carries the RUT/VARIED verdict -- the PM-obeys-RUT wiring LANDED and editing that core moves a running loop's semantics:
...
uv run python foundry.py stage-times  # [--log PATH] [--team NAME] [--budget N seconds] [--limit N most-recent iterations, windowed AFTER the --team filter; 0/negative/omitted mean all] [--json for one machine-readable digest; same 0/1/2 exit code]
...
uv run python foundry.py live-lag --config products/_platform/config.json  # [--log PATH to the dispatcher log carrying the `dispatcher up` banner; default the foundry checkout's own dispatcher.out] [--json for one machine-readable live-lag verdict doc: dashboards/CI/restart-wrapper; same 0/2 exit code, and the payload's `exit_code` is the process exit code]
...
# 48. PRICE THE RULE THE LOOP SURVIVES ON (`rescues`): a PER-STAGE accounting of agent-CLI hard kills -- attempts / kills / RESCUED / LOST / rescue rate -- read from the `iter-NN/<stage>.attemptN.log` files `run_stage` has written since the beginning and that NOTHING in this module has ever read (its only other appearance is the WRITE site). The ~600s per-stage kill is the #1 documented cause of lost shifts and the loop survives it ONLY because every role card orders the stage to checkpoint its output file early -- `run_stage` returns success on a non-empty file whatever the child did next -- yet nothing measured how often that rescue fires. #42 `stage-times` structurally cannot: it records a killed-but-checkpointed attempt as `produced=True, kind=""`, indistinguishable from one that finished at 598s, so it reported 45 failed attempts over the same window whose attempt logs hold 232 hard kills (223 rescued, 9 lost, every loss an `engineer` attempt) -- a 5.2x under-count -- and its `STAGE_SOFT_BUDGET` WARN fires for 9 of 10 live `_platform` stages, so it supplies no signal either. A kill is DATA, not a buried literal: `ATTEMPT_KILL_TOKENS` (the agent CLI's own `agent run timed out after`, whose wording is not ours to control) and `ATTEMPT_LOG_GLOB` are module-level and read at call time. `rescued` mirrors `run_stage`'s own success test byte-for-byte (the stage's OUTPUT file exists with `st_size > 0`, resolved through the module-level patchable `STAGE_OUTPUT_NAMES` because `run_stage` takes `out_name` SEPARATELY from the stage label that names the log -- they differ for `fix-review`, `fix-tests`, `tester-rerun`, `tester-retry`, `tester-retry2` and `reporter`, and assuming `<stage>.md` reported their 100.0% rescue rate as 0.0%), so a rescue here means exactly what it meant to the loop; an unreadable/undecodable log counts as an attempt that is NOT a kill, because absence of evidence is not evidence and dropping it would shrink the denominator the rate is computed against. A killed-but-RESCUED attempt is deliberately NOT a failure (gating on it would make a healthy loop look broken every shift) -- only LOST work gates. Pure core + ONE read-only seam, dormant: the pipeline/dispatcher never call it and it writes nothing (exit 0 nothing lost / 1 >=1 LOST attempt / 2 nothing to scan):
uv run python foundry.py rescues --config products/_platform/config.json  # [--limit N most-recent iterations] [--json for one machine-readable accounting doc: dashboards/cron; same 0/1/2 exit code, honours --limit]

# 49. START A TEAM ON A REPO (`new-product`): write `products/<name>/config.json` from arguments, so onboarding a product is one command instead of hand-copying a sibling config and hand-editing every field. It REFUSES to clobber an existing config (exit 2) unless `--force` is passed, because a config is a running team's wiring and silently overwriting one re-points a live loop at a different repo. `--allowed-push-repo` defaults to the last component of `--repo`, which is the push-target guard the final gate enforces:
uv run python foundry.py new-product --name my-thing --repo ~/projects/my-thing  # [--branch main] [--test-cmd 'uv run pytest'] [--allowed-push-repo NAME] [--force to overwrite an existing config]

# 50. RE-VERIFY THE SHIP COMMIT FROM A CLONE (`preship`): the BLOCKING pre-push gate `roles/final.md` runs between commit and push -- it clones the LOCAL repo at the ship commit into a throwaway directory and runs the product's own quality command there, so a test that passes only because of GITIGNORED working-tree state fails HERE instead of after the push (iter 154 shipped exactly that bug). Bounded by one `PRESHIP_BUDGET_SECONDS` knob. Exit 1 (suite failed / sha mismatch) BLOCKS the push; exit 2 (clone, install or budget) is ADVISORY on purpose -- `postrelease_step` is still the backstop, and a false block would destroy a green iteration:
uv run python foundry.py preship --config products/_platform/config.json  # [--json for one machine-readable verdict: dashboards/CI; same 0/1/2 exit code]

# 51. NAME THE CAUSE OF THE WORK THIS FRAMEWORK DESTROYED (`losses`): a per-CAUSE accounting of the stage attempts that produced NO output file, classified with the shipped `classify_attempt_failure` labels -- `timeout` (the bite did not fit the ~600s cap) / `stalled` (a locally-silent command tripped the 120s no-output detector) / `cli-error` (the agent app rotated its IPC socket out from under the stage) / `service` (a throttled backend) / `auth` (the session expired, so only a human re-authenticating heals it) / `other` -- read from the same `iter-NN/<stage>.attemptN.log` files #48 `rescues` reads. WHY BOTH VERBS EXIST: `rescues` decides "this attempt lost work" from `ATTEMPT_KILL_TOKENS`, a SINGLE token (`agent run timed out after`), so measured on this repo it counts 9 of the 64 `_platform` attempts that produced nothing -- 14.1% -- and reports `[tester] ... lost 0` and `[final] ... lost 0` while 38 tester attempts and iteration 172's ENTIRE release gate were lost. Its ratio was never wrong; the population it ranges over was, and a gauge whose denominator holds work its numerator cannot see reads as HEALTH -- fail-open monitoring, which is worse than none. The SPLIT is the feature, because each kind has a DIFFERENT owner: a `timeout` is a PM decision (shrink the bite), a `stalled` is a tester decision (no command may run silent for 120s), a `cli-error` is a human relaunch and owes the loop no change at all, and a `service` means wait -- one undifferentiated number can be assigned to nobody. `stages` on each row names WHERE that cause bites (`stalled` is 38 of 38 `tester`) without paying for a full cause x stage cross-tab. Pure `attempt_loss_summary` core + ONE read-only `gather_losses` seam, dormant: the pipeline/dispatcher never call it and it writes nothing (exit 0 nothing lost / 1 >=1 LOST attempt / 2 nothing to scan):
uv run python foundry.py losses --config products/_platform/config.json  # [--limit N to scan only the most-recent N iterations] [--json for one machine-readable document: dashboards/CI; same 0/1/2 exit code]

# 52. READ THE STOP SENTINELS THE FLEET REPORT CANNOT SEE (`company-stops`): the RETIREMENT axis of #16 `company-status`, which is structurally blind to it -- a team's health is derived from the newest artifact that team ever WROTE, so a team deliberately retired stops changing its own health and keeps reporting the last thing it said. Measured on this repo's own roster: a team whose `STOP` sentinel had sat on disk for 27 days reported `verdict: "OK"`, `ok: true`, `postrelease: "HEALTHY"`, with the token STOP absent from the entire payload, while its roster entry was still `"enabled": true` -- the 2026-08-19 lesson in the tree, that a freshness check over an AGGREGATE cannot detect a dead COMPONENT. The two sentinels are the dispatcher's own contract (`<foundry>/STOP` stops the whole company, `<work_root>/STOP` retires one team) and before this verb their only readers in the module were `global_stop()` / `stopping(cfg)`, which no verb calls -- so the only way to learn a team was retired was to `ls` two paths by hand. It reports the sentinel's own REASON and not just the boolean, because these files carry a reactivation condition an operator must read BEFORE deleting one. EXISTENCE DECIDES THE STOP AND TEXT ONLY DECIDES THE REASON: an unreadable or undecodable sentinel still reports stopped, with a marker reason, rather than becoming an error row -- the alternative fails OPEN on exactly the file whose bytes went bad. Global scope is folded into the ROW rather than reported once at company level, so a company-wide stop can never leave every row reading `stopped: false` while nothing runs, and when both sentinels exist the GLOBAL reason wins because global scope is strictly broader. The TENTH member of the `company-*` family and thin by reuse -- the shared roll-up body already owns the roster walk, the `{FOUNDRY}` substitution, the disabled skip and both error paths, so a disabled work item is never loaded and one bad team is recorded while the roll-up continues. Sentinel AGE in days is deliberately absent: it needs a clock, which would make every row time-dependent. Pure `stop_reason` core + ONE read-only `gather_stop` seam, dormant: the pipeline/gate/dispatcher never call it and it writes nothing (exit 0 nothing stopped / 1 a team is stopped or a team errored / 2 nothing to report):
uv run python foundry.py company-stops  # --config points at the DISPATCH config (foundry.config.json), NOT a product config (default: the repo's foundry.config.json) [--json for one machine-readable company roll-up doc: dashboards/cron; same 0/1/2 exit code]
