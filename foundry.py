...
# --------------------------------------------------------------------------- #
# The provenance marker every `render_agents_md` output carries, hoisted to a
# module constant so `agents_doc_is_generated` can recognise the foundry's OWN
# output without holding a second copy of the sentence: one literal, two readers.
# A doc that lacks it is somebody's hand-written file -- which is exactly what
# makes the default write non-destructive (iter 182).
AGENTS_GENERATED_BANNER = (
...
def agents_doc_is_generated(text: str) -> bool:
    """True iff ``text`` carries the banner every ``render_agents_md`` output has.

    PROVENANCE, not difference -- this is the distinction that lets `foundry agents`
    regenerate its OWN output freely while refusing to clobber a hand-written
    ``AGENTS.md``. A difference-based rule ("refuse when the file differs from what
    I would write now") cannot express that: a legitimate refresh exists precisely
    to change the file, so difference would refuse every real use of the verb.

    Pure -- no filesystem/subprocess/network/clock -- so the decision is offline
    testable on plain strings, and total: any ``str`` (including ``""``) yields a
    bool rather than raising. Reading the file is ``agents_cli``'s job, not this
    predicate's.
    """
    return AGENTS_GENERATED_BANNER in text
...
class AgentsView:
    ...
    def to_dict(self) -> dict:
        ...
        directly). No ``exit_code`` key: this envelope is the READ-ONLY ``--json``
        mode, which always returns 0. The CLI as a whole DOES have an error path
        (the default write refuses a hand-written target with 2), but a refusal
        never renders a view, so no view can ever carry a non-zero code.
        """
        return {
...
def agents_cli(cfg: ProductConfig, recent: int = 12,
               print_only: bool = False, as_json: bool = False,
               force: bool = False) -> int:
    """CLI entry: render a product's ``AGENTS.md`` doc; 0, or 2 when it refuses.
    ...
    ``print_only`` (it writes NO file and creates no repo dir).

    The default write is NON-DESTRUCTIVE: it refuses with 2, having written ZERO
    bytes, when the target already exists and does NOT carry
    ``AGENTS_GENERATED_BANNER`` -- i.e. it is somebody's hand-written doc rather
    than our own output. Regenerating a banner-bearing file is always allowed (a
    refresh is the whole point of the verb), and ``force`` overwrites regardless.
    The two READ-ONLY modes never refuse and never write, so they still always
    return 0; only the default write can return 2.
    """
    path = pathlib.Path(cfg.learnings)
    text = path.read_text() if path.exists() else ""
    if as_json:
    ...
    if target.exists() and not force:
        try:
            ...
            # Unreadable, undecodable, or a directory: it cannot be carrying our
            # banner, so treat it as somebody else's artifact and refuse. Fail
            # CLOSED -- the cost of a needless refusal is one `--force`, the cost
            # of a wrong overwrite is a destroyed hand-written file.
            existing = ""
        if not agents_doc_is_generated(existing):
            print(f"agents: {target} appears hand-written (no foundry-generated "
                  "banner); refusing to overwrite it (re-run with --force to "
                  "replace it)")
            return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    agt.add_argument("--force", action="store_true",
                     help="overwrite <repo>/AGENTS.md even when it looks hand-written")
