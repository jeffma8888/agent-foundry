...
6. **Leak-guard clean (public-safety, repo-agnostic).** If the repo carries the committed leak-guard (guard on `[ -f <repo>/scripts/leak_guard.py ]`), it MUST scan the commit you are about to push and find nothing. This runs in the ship flow below, AFTER the commit and BEFORE the push: `python3 <repo>/scripts/leak_guard.py --ref HEAD --repo <repo>`. PREFER the verb, which reports a WORD as well as a code: `python3 <checkout>/foundry.py leak-check --config PRODUCT_CONFIG` runs that same scanner and prints ONE line `leak-guard: CLEAN|LEAKED|UNKNOWN|ABSENT -- <detail>` with the SAME fail-closed 0/1/2 exit code, so you never have to read the verdict off shell plumbing alone (a bash-only `${PIPESTATUS[0]}` expanded to the empty string under zsh once and left this gate's verdict unreadable). `CLEAN`/`ABSENT` pass; `LEAKED` and `UNKNOWN` -- the scan could not be decided -- both BLOCK. A non-zero exit is a BLOCKING gate failure, fail-CLOSED: BOTH exit 1 (a leaked internal or personal token was found) AND exit 2 (the scanner could not complete or errored) mean do NOT push and go to the "If ANY fail" revert path below. Never let the guard be defeated by making it error past. If the repo does NOT carry `scripts/leak_guard.py` (most products do not), SKIP this check: its absence is not a gate failure.
...
- **Leak-guard gate (belt-and-suspenders, before the push).** If the repo carries `<repo>/scripts/leak_guard.py`, run `python3 <repo>/scripts/leak_guard.py --ref HEAD --repo <repo>` now: it scans the commit tree you just made. The verb `python3 <checkout>/foundry.py leak-check --config PRODUCT_CONFIG` is the same scan with a readable verdict -- one `leak-guard: <VERDICT> -- <detail>` line plus the same exit code -- and is the invocation to use; grade the WORD and the code together. A non-zero exit (1 = leaked token, 2 = scanner error; fail-CLOSED) means abort the push and go to the "If ANY fail" revert path below. If the repo has no `scripts/leak_guard.py`, skip this. The installed git `pre-push` hook is the primary block; this is the second.
