"""Black-box behaviour tests for iter 54 -- `foundry company-constant-asserts`
(a VERBATIM RETRY of iter 53): the read-only, offline company-wide roll-up of the
per-product iter-48 `constant-asserts` scan -- the 6th and FINAL `company-*`
family member (after company-status/history/timing/weak-tests/events). It folds
every ENABLED dispatch team's constant-assert scan into ONE report + a scriptable
GATE exit code, composed on top of the shipped `gather_constant_asserts` seam +
the frozen `ConstantAssertSummary`. Purely additive in foundry.py:

  * a FROZEN dataclass `CompanyConstantAsserts(dispatch_path, products, disabled,
    ...
    render() + to_dict(),
  * a PURE keyword-only `summarize_company_constant_asserts(*, dispatch_path,
    products, disabled, errors) -> CompanyConstantAsserts`,
  * a thin resilient `company_constant_asserts_cli(dispatch_path, as_json=False)
    -> int` wired to a new argparse subcommand `company-constant-asserts` (NO
    --limit, NO --files), reusing the shipped `parse_dispatch_work_items` +
    `gather_constant_asserts` + `ConstantAssertSummary` seams.

ISOLATION CONTRACT (HONORED): this file was written ONLY from the iter-54 PM
spec's Expected Behaviors (1-8), the product README/roadmap, the tests/
conventions (esp. tests/test_iter43_behavior.py -- the structural mirror
company-weak-tests -- and tests/test_iter48_behavior.py -- the per-product
constant-asserts foundation), and the product's own OBSERVABLE behaviour
(building the public objects and RUNNING them / --help). The implementation
SOURCE (foundry.py / dispatcher.py source text), the engineer's & reviewer's
notes, and `git diff` were NOT read. Every assertion is DERIVED from the spec's
pinned substrings, never copied from optional implementation phrasing. Every
check drives the PUBLIC interface against tiny JSON files in tmp_path,
monkeypatching foundry.load_config / foundry.gather_constant_asserts. The real
...
# the genuinely-NEW iter-54 symbols
NEW_SYMBOLS = ("CompanyConstantAsserts", "summarize_company_constant_asserts",
               "company_constant_asserts_cli")
CONTROL_FLOW_FNS = ("build_prompt", "run_stage", "run_iteration", "run_continuous")
SUBCMD = "company-constant-asserts"


# --------------------------------------------------------------------------
# helpers  (mirror tests/test_iter43_behavior.py)
# --------------------------------------------------------------------------
def _cs(product, files_scanned, findings=(), parse_errors=()):
    """Build a per-product ConstantAssertSummary via the shipped pure factory."""
...
def _run_cca(dispatch_path, as_json=False):
    """Drive company_constant_asserts_cli directly, capturing (rc, stdout+stderr)."""
    out, err = io.StringIO(), io.StringIO()
    old_out, old_err = sys.stdout, sys.stderr
    sys.stdout, sys.stderr = out, err
    try:
        rc = foundry.company_constant_asserts_cli(dispatch_path, as_json=as_json)
    finally:
...
def _patch_cli(monkeypatch, cs_by_name):
    """Monkeypatch load_config (tags a cfg with the resolved path, records every
    load) + gather_constant_asserts (returns a ConstantAssertSummary by matching a
    ...
    def fake_gather(cfg, files=None):
        for name, cs in cs_by_name.items():
            if name in cfg._path:
                return cs
        return _cs("unknown", 1)

    monkeypatch.setattr(foundry, "load_config", fake_load)
    monkeypatch.setattr(foundry, "gather_constant_asserts", fake_gather)
    return loaded
...
def test_b1_worked_example_fields_sums_and_counts():
    pA = _cs("alpha", 3, findings=(("a.py", "test_x"),), parse_errors=())
    pB = _cs("beta", 2, findings=(), parse_errors=(("b.py", "SyntaxError: bad"),))
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d/fc.json", products=(pA, pB),
        disabled=("gamma", "delta"), errors=(("eps", "boom"),))
    assert dataclasses.is_dataclass(cca)
    assert type(cca).__name__ == "CompanyConstantAsserts"
    assert cca.dispatch_path == "/d/fc.json"
    assert cca.products == (pA, pB) and cca.disabled == ("gamma", "delta")
    assert cca.errors == (("eps", "boom"),)
    assert cca.files_scanned == 5 == sum(p.files_scanned for p in cca.products)
    assert cca.total_findings == 1 == sum(p.total_findings for p in cca.products)
    assert cca.total_parse_errors == 1 == sum(len(p.parse_errors) for p in cca.products)
    assert cca.n_flagged == 2
    assert cca.n_products == 2 == len(cca.products)
    assert cca.n_disabled == 2 == len(cca.disabled)
    assert cca.n_errors == 1 == len(cca.errors)
...
def test_b1_coerces_iterables_to_tuples_keyword_only():
    ...
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=[_cs("a", 1)], disabled=["x"],
        errors=[("e", "m")])
    assert isinstance(cca.products, tuple)
    assert isinstance(cca.disabled, tuple) and cca.disabled == ("x",)
    assert isinstance(cca.errors, tuple) and cca.errors == (("e", "m"),)


def test_b1_is_frozen():
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=(_cs("a", 1),), disabled=(), errors=())
    with pytest.raises(dataclasses.FrozenInstanceError):
        cca.dispatch_path = "/other"


def test_b1_empty_products_all_zero():
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=(), disabled=(), errors=())
    assert cca.files_scanned == 0 and cca.total_findings == 0
    assert cca.total_parse_errors == 0 and cca.n_flagged == 0
    assert cca.n_products == 0 and cca.n_disabled == 0 and cca.n_errors == 0
...
def test_b2_n_flagged_counts_findings_or_parse_errors_only():
    p_find = _cs("f", 2, findings=(("x.py", "test_x"),))          # flagged
    p_parse = _cs("p", 2, parse_errors=(("y.py", "SyntaxError: y"),))  # flagged
    p_clean = _cs("c", 5)                                         # NOT flagged
    p_zero = _cs("z", 0)                                          # NOT flagged
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=(p_find, p_parse, p_clean, p_zero),
        disabled=(), errors=())
    assert cca.n_products == 4
    assert cca.n_flagged == 2, \
        "only products with findings OR parse-errors are flagged (not clean/zero-file)"
    assert cca.files_scanned == 2 + 2 + 5 + 0 == 9
    assert cca.total_findings == 1
    assert cca.total_parse_errors == 1


# ==========================================================================
# Behavior 3 -- exit_code (findings-first GATE) + verdict token
# ==========================================================================
def test_b3_exit_and_verdict_matrix():
    cases = (
        ((_cs("a", 5),), (), 0),                                          # clean
        ((_cs("a", 3, findings=(("t.py", "test_a"),)),), (), 1),         # findings
        ((_cs("a", 1, parse_errors=(("t.py", "SyntaxError: b"),)),), (), 1),  # parse
        ((_cs("a", 5),), (("boom", "load failed"),), 1),                 # struct err
        ((), (), 2),                                                     # no products
    ...
    for products, errors, code in cases:
        ...
        assert cca.exit_code == code, (products, errors, cca.exit_code)
        assert cca.verdict == _VERDICT_FOR_CODE[code] == _VERDICT_FOR_CODE[cca.exit_code]


def test_b3_zero_file_product_does_not_force_exit2():
    zf = _cs("zf", 0)
    assert zf.exit_code == 2, "sanity: a zero-file ConstantAssertSummary is itself exit 2"
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=(zf,), disabled=(), errors=())
    assert cca.n_products == 1 and cca.files_scanned == 0
    assert cca.exit_code == 0 and cca.verdict == "clean", \
...
def test_b4_render_header_path_counts_rollup_verdict():
    pA = _cs("alpha", 3, findings=(("a.py", "test_x"),))
    pB = _cs("beta", 2, parse_errors=(("b.py", "SyntaxError: bad"),))
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d/fc.json", products=(pA, pB),
        disabled=("gone",), errors=(("errp", "boom msg"),))
    r = cca.render()
    assert "foundry company-constant-asserts" in r
    assert "/d/fc.json" in r, "render must contain the dispatch path"
    assert f"{cca.n_products} gathered" in r
    assert f"{cca.n_disabled} disabled" in r
    assert f"{cca.n_errors} error" in r
    assert f"{cca.files_scanned} files scanned" in r
    assert f"{cca.total_findings} constant-assert tests" in r
    assert f"{cca.total_parse_errors} parse errors" in r
    assert _final_verdict_token(r) == cca.verdict == "ATTENTION"


def test_b4_render_one_line_per_gathered_product_with_own_counts():
    pA = _cs("alpha", 3, findings=(("a.py", "test_x"), ("a2.py", "test_y")))
    pB = _cs("beta", 2, parse_errors=(("b.py", "SyntaxError: bad"),))
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=(pA, pB), disabled=(), errors=())
    la = _product_line(cca.render(), "alpha")
    assert f"{pA.files_scanned} files scanned" in la
    assert f"{pA.total_findings} constant-assert" in la
    assert f"{len(pA.parse_errors)} parse error" in la
    lb = _product_line(cca.render(), "beta")
    assert f"{pB.files_scanned} files scanned" in lb
    assert f"{pB.total_findings} constant-assert" in lb
    assert f"{len(pB.parse_errors)} parse error" in lb


def test_b4_render_disabled_and_error_lines():
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=(_cs("okp", 5),),
        disabled=("dis1", "dis2"), errors=(("errp", "kaboom message here"),))
    lines = cca.render().splitlines()
...
def test_b4_render_no_leaf_leak_and_deterministic():
    ...
    pA = _cs("alpha", 3, findings=(("secretfile.py", "test_secretfn"),),
    ...
    assert cca.render() == cca.render(), "render() must be deterministic"
...
def test_b5_to_dict_keys_roundtrip_and_values():
    ...
    assert d["dispatch_config"] == "/d/fc.json" == cca.dispatch_path
    assert d["disabled"] == ["gamma"]
    assert d["errors"] == [{"product": "eps", "message": "boom"}]
    assert (d["n_products"], d["n_disabled"], d["n_errors"], d["n_flagged"]) == \
        (cca.n_products, cca.n_disabled, cca.n_errors, cca.n_flagged) == (2, 1, 1, 2)
    assert (d["files_scanned"], d["total_findings"], d["total_parse_errors"]) == \
        (cca.files_scanned, cca.total_findings, cca.total_parse_errors) == (5, 1, 1)
    assert d["exit_code"] == cca.exit_code == 1
    assert d["verdict"] == cca.verdict == "ATTENTION"
...
def test_b5_to_dict_products_carry_full_per_product_leaf_detail():
    ...
    assert d["products"] == [pA.to_dict(), pB.to_dict()], \
        "products must be each gathered ConstantAssertSummary.to_dict() IN ORDER"
...
def test_b5_to_dict_all_empty_serializes_without_raising():
    cca = foundry.summarize_company_constant_asserts(
        dispatch_path="/d", products=(), disabled=(), errors=())
    js = json.loads(json.dumps(cca.to_dict()))
...
# ==========================================================================
# Behavior 6 -- company_constant_asserts_cli rolls up an enabled dispatch config
# ==========================================================================
def test_b6_enabled_gathered_disabled_recorded_never_loaded(tmp_path, monkeypatch):
    ...
    loaded = _patch_cli(monkeypatch, {"alpha": _cs("alpha", 3, findings=(("a.py", "t"),))})
    rc, out = _run_cca(str(disp))
...
def test_b6_clean_enabled_team_exits_0(tmp_path, monkeypatch):
    ...
    _patch_cli(monkeypatch, {"alpha": _cs("alpha", 5)})
    rc, out = _run_cca(str(disp))
...
def test_b6_foundry_token_substituted_before_load(tmp_path, monkeypatch):
    ...
    loaded = _patch_cli(monkeypatch, {"alpha": _cs("alpha", 3)})
    _run_cca(str(disp))
...
def test_b6_gather_called_with_cfg_via_bare_name_seam(tmp_path, monkeypatch):
    ...
    assert seen == [str(tmp_path / "alpha.json"), str(tmp_path / "beta.json")], \
        f"gather_constant_asserts must be called by BARE name per enabled cfg: {seen}"
...
def test_b6_json_is_single_indent2_document_same_exit(tmp_path, monkeypatch):
    ...
    _patch_cli(monkeypatch, {"alpha": _cs("alpha", 3, findings=(("a.py", "t"),)),
                             "beta": _cs("beta", 2)})
    rc_h, _ = _run_cca(str(disp), as_json=False)
    rc_j, out_j = _run_cca(str(disp), as_json=True)
...
def test_b6_human_path_prints_render(tmp_path, monkeypatch):
    ...
    csA = _cs("alpha", 3, findings=(("a.py", "t"),))
    csB = _cs("beta", 2)
    _patch_cli(monkeypatch, {"alpha": csA, "beta": csB})
    rc, out = _run_cca(str(disp), as_json=False)
    expected = foundry.summarize_company_constant_asserts(
        dispatch_path=str(disp), products=(csA, csB), disabled=(), errors=())
...
def test_b6_read_only_writes_nothing_to_disk(tmp_path, monkeypatch):
    ...
    _patch_cli(monkeypatch, {"alpha": _cs("alpha", 3)})
    before = _snapshot_tree(tmp_path)
    _run_cca(str(disp), as_json=False)
    _run_cca(str(disp), as_json=True)
    assert _snapshot_tree(tmp_path) == before, \
        "company-constant-asserts wrote to disk (must be read-only)"
...
def test_b7_missing_dispatch_file_exit1_no_raise(tmp_path):
    rc, out = _run_cca(str(tmp_path / "does-not-exist.json"))
...
def test_b7_one_bad_team_recorded_rollup_continues(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(foundry, "load_config", fake_load)
    monkeypatch.setattr(
        foundry, "gather_constant_asserts",
        lambda cfg, files=None: _cs(
            "good" if "good.json" in cfg._path else "good2", 3))
    rc, out = _run_cca(str(disp), as_json=True)
...
def test_b8_default_config_is_repo_dispatch_config(monkeypatch):
    ...
    monkeypatch.setattr(foundry, "company_constant_asserts_cli", spy)
...
def test_b8_json_flag_passes_through(monkeypatch):
    captured = {}
    monkeypatch.setattr(
        foundry, "company_constant_asserts_cli",
...
def test_b8_dispatched_before_load_config(monkeypatch):
    monkeypatch.setattr(foundry, "company_constant_asserts_cli",
...
def test_b8_new_surface_present_and_reuses_shipped_seams():
    ...
    assert callable(foundry.summarize_company_constant_asserts)
    assert callable(foundry.company_constant_asserts_cli)
    for name in ("parse_dispatch_work_items", "gather_constant_asserts",
                 "ConstantAssertSummary", "summarize_constant_asserts",
                 "constant_asserts_cli", "find_constant_assert_tests",
                 "load_config"):
...
def test_b8_help_lists_company_constant_asserts_after_siblings(capsys):
    with pytest.raises(SystemExit) as ei:
        foundry.main(["--help"])
    assert ei.value.code == 0
    out = capsys.readouterr().out
    for sub in ("constant-asserts", "company-status", "company-history",
                "company-timing", "company-weak-tests", "company-events", SUBCMD):
        assert sub in out, f"subcommand {sub!r} missing from --help:\n{out}"


def test_b8_constant_assert_summary_to_dict_still_8_keys_unchanged():
    s = foundry.summarize_constant_asserts(
        product="p", files_scanned=2, findings=(("t.py", "test_a"),), parse_errors=())
    d = s.to_dict()
    assert len(d) == 8, f"ConstantAssertSummary.to_dict must still have 8 keys: {list(d)}"
    assert list(d.keys()) == [
...
def test_b8_no_surviving_foundry_py_byte_unchanged_assertion():
    # spec-mandated prior-iter maintenance: NO test may still pass foundry.py as a
    # `git diff --quiet` byte-unchanged arg (a routinely-extended file is a latent
    # suite-breaker for the next iteration that touches it). A real assertion passes
    # the QUOTED arg token "foundry.py"/'foundry.py" inside a subprocess arg list;
    # scan only files that actually contain a `git diff --quiet` call, skip comment
    # lines, and self-exclude this meta-scanner (its detection logic references the
    # tokens as string literals, not as a git-diff arg).
    ...
    for f in sorted(tests_dir.glob("test_*.py")):
        ...
        for i, line in enumerate(text.splitlines(), 1):
            if line.lstrip().startswith("#"):
                continue
            if '"foundry.py"' in line or "'foundry.py'" in line:
                offenders.append(f"{f.name}:{i}: {line.strip()}")
    assert not offenders, \
        "foundry.py must not be asserted byte-unchanged anymore:\n" + "\n".join(offenders)
...
def test_b8_shipped_public_files_scan_clean_and_no_home_path():
    ...
    for rel in ("tests/test_iter54_behavior.py", "README.md", "PLATFORM_ROADMAP.md"):
        p = _ROOT / rel
        if not p.exists():
            continue
        txt = p.read_text()
